...
def dizhumain(s):
    ...
    def greater_than(lzuikai, c): 
     # 传入手拍列表，和上次人出牌，请玩家在手中选择，比上次牌大的，或者跳过不出牌
        while True:
            print("能管上吗?")
            t = threading.Thread(target=daojishi)  #创建线程进行倒计时
            t.start()
            L1 = click(lzuikai)         #选择的要出的牌
            t.join()
            a = '^'.join(L1)            #将出牌列表拼接成字符串
            if a == '':        #表示选择不出
                # background = pygame.image.load(background_image).convert()
                ...
                if jian2(a, c):            #检查出牌是否符合规则
                    print("zhen")
                    chuqu(lzuikai, a)       #从手牌中删除打出去的牌

                    lgangchu = L1.copy()    #用于发送给服务器

                    x1 = 0           #？？？？？
                    l1 = a.split('^')
                    print("您剩余牌为", paixu("^".join(lzuikai)))
                    sheng = len(lzuikai)    #剩余手牌张数
                    sockfd.send(
                    ...
                    if len(lzuikai) == 0:
                        sleep(0.2)
                        sockfd.send(
                            ("地主^客户^结算一^胜利^"+dizhufangjianhao+\
                                "^"+benrenshenfen+"^"+nameid).encode())
                        dizhushenglihui()
                        return
    ...
    def greater_than2(lzuikai, c):       #上一个函数，用于下一家出牌
     # 传入手拍列表，和上次人出牌，请玩家在手中选择，比上次牌大的，或者跳过不出牌
        while True:
            print("能管上吗?")
    ...
    def jian1(a):  # 检测随意出牌,但要符合出牌基本规则
        ...
        if len(l) == 2:                 
            ...
            return False
        if len(l) == 3:               
            if l[0][2:] == l[1][2:] and l[0][2:] == l[2][2:]:     #3不带
                return True
            return False
        if len(l) == 4:
            if (l[0][2:] == l[1][2:] and l[0][2:] == l[2][2:]):    #3带一个大
                return True
            if (l[1][2:] == l[2][2:] and l[1][2:] == l[3][2:]):    #3带一个小
                return True
    ...
    def jian2(a, b):  # 检测是否符合基本出牌原则,是否比上一家大  #a要出的牌 ，b上家出的牌
        if not jian1(a):
            return False
        l1 = paixu(a).split("^")
        l2 = paixu(b).split("^")
        ...
        if len(l1) == 1 and len(l2) == 1:               #单牌
            if int(l1[0][2:]) > int(l2[0][2:]):
                return True
            return False
        if len(l1) == 2 and len(l2) == 2:             #对子
            if int(l1[0][2:]) > int(l2[0][2:]):
                return True
            return False
        if len(l1) == 3 and len(l2) == 3:               #三不带
            if int(l1[0][2:]) > int(l2[0][2:]):
                return True
            return False
        if len(l1) == 4 and len(l2) == 4:              #三带一      
